...
struct AnimatedGradientBackground: View {
    ...
    func interpolatedColors(from: [SIMD3<Double>], to: [SIMD3<Double>], progress: CGFloat) -> [Color] {
...
private extension Color {
    var components: (red: CGFloat, green: CGFloat, blue: CGFloat) {
        ...
        if let cgColor = self.cgColor,
           let nsColor = NSColor(cgColor: cgColor)?.usingColorSpace(.deviceRGB) ?? NSColor(cgColor: cgColor)?.usingColorSpace(.sRGB) {
            var red: CGFloat = 0
            var green: CGFloat = 0
            var blue: CGFloat = 0
            var alpha: CGFloat = 0
        ...
        if let provider = mirror.children.first(where: { $0.label == "provider" })?.value {
            ...
            if let base = providerMirror.children.first(where: { $0.label == "base" })?.value {
                ...
                if let red = baseMirror.children.first(where: { $0.label == "red" })?.value as? Double,
                   let green = baseMirror.children.first(where: { $0.label == "green" })?.value as? Double,
                   let blue = baseMirror.children.first(where: { $0.label == "blue" })?.value as? Double {
